...
function aFunction() {
	...
	//different ways to write data to webpage
...
function switchFunction() {
	...
	let operand = "*"; //could set to any operand sign
	...
	function forLoopFunction(){
		for(let index = 10; index >=0; --index){
			document.write(index + "<br>"); // write creates new page so don't need 
		}
	}
	
	function doWhileLoopFunction(){
		let number = prompt("Enter number");
		number  = parseInt(number); //js reads number entered as a string so we change into a number
		do{
			document.write("Printing <br>");
			number++;
		}
		while(number < 10); //enter name and password is a good use ... validate in while clause
	}
